...
void Init(void)
{
	...
	//ゲーム起動時に一度だけ呼ばれる
	//-----------------------------------------------------
	//マップエディタ[plutinum]がbmpの対応のため.bmp画像を使用
	ADD_RESOURCE("Yuka", CImage::CreateImage("Image/Yuka.bmp"));
	ADD_RESOURCE("Kabe", CImage::CreateImage("Image/Kabe.bmp"));
	ADD_RESOURCE("Player", CImage::CreateImage("Image/muramusume.png", player_anim_data, 48, 48));





	//プレイヤーの生成 位置はあとでMapが決める
	Base::Add(new Player());
